    ...
    try:
        ...
        query = """
            SELECT 
                p.id_penyakit, 
                p.nama_penyakit, 
                COUNT(a.id_gejala) AS jumlah_gejala_cocok
            FROM aturan a
            ...
            HAVING 
                COUNT(a.id_gejala) = (
                    SELECT COUNT(*) 
                    FROM aturan 
                    WHERE id_gejala IN %s AND id_penyakit = p.id_penyakit
                )
                AND COUNT(a.id_gejala) = (
                    SELECT COUNT(*) 
                    FROM aturan 
            ...
            LIMIT 1;
        """
        gejala_tuple = tuple(gejala)
        cur.execute(query, (gejala_tuple, gejala_tuple))
        print(f"Gejala: {gejala_tuple}")
        hasil = cur.fetchone()
        cur.close()
        if hasil:
            return hasil[1]  # Mengembalikan hanya nama penyakit
        return None
    except Exception as e:
